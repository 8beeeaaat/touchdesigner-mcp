...
1. Read the TouchDesigner host, port, and endpoint from the `touchdesigner configuration` context injected at session start. `get_td_info` takes no connection parameters because the bundled MCP server receives the same values through its startup arguments. If the configuration context is missing, use `http://127.0.0.1:9981` as the fallback and tell the user the plugin's SessionStart hook may be disabled.
