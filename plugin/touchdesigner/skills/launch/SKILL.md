...
1. **Check whether launching is even needed.** Read the endpoint from the `touchdesigner configuration` context injected at session start, falling back to `http://127.0.0.1:9981` if that context is missing. Call `get_td_info`. If it succeeds, TouchDesigner is already up with the component loaded — report the versions and stop. This makes the command idempotent. If it fails and the configured host is not a loopback host, stop here: this command can launch only a local TouchDesigner application, so ask the user to start TouchDesigner on the configured remote machine and continue with `/touchdesigner:setup` instead.
