...
3. Traverse the COMP hierarchy breadth-first starting at `root`, using repeated `get_td_nodes` calls (`parentPath` set to each COMP-family node discovered so far, `includeProperties: false` for speed). Recurse only into children whose `opType` ends in `COMP`, since those are the containers that can hold further children. Bound the traversal to a sensible depth (e.g. 4 levels) and to a sensible per-level node count; if either bound is hit, note explicitly in the report that the tree was truncated at that point rather than silently dropping nodes.

4. While traversing, tally node counts per operator family by matching each `opType` string's suffix (`TOP`, `CHOP`, `SOP`, `DAT`, `MAT`, `COMP`) — this is a naming convention, not a separate field, so classify accordingly.
