...
- `references/operator-guide.md` — a curated index of the most-used operators per family (verified `nodeType` strings, one-line purposes) plus the live-lookup procedure for fetching any operator's parameter names, defaults, menus, and official help text from the running TouchDesigner on demand. Parameter details are deliberately not stored statically — fetch them live.
