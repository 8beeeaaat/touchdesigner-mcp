...
- **`IndexError` on a channel index** — indexing a CHOP like `op('noise1')[0]` fails when the operator has fewer channels than expected, or none at all (for instance because an upstream node isn't cooking, or produced zero channels due to a misconfigured parameter). Debug by first inspecting the operator's actual channel count and names through `get_td_node_parameters` rather than assuming the channel layout from the operator's typical default configuration.
