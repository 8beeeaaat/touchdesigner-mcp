...
1. **Check whether launching is even needed.** Call `get_td_info`. If it succeeds, TouchDesigner is already up with the component loaded — report the versions and stop. This makes the command idempotent.
...
3. **Locate the TouchDesigner application.**
   ...
   - Windows: newest `TouchDesigner.exe` under `%ProgramFiles%\Derivative\TouchDesigner*\bin\`.
...
4. **Launch with the tox as the opened document** (a Bash permission prompt is expected here — this skill deliberately does not pre-authorize shell commands):
   ...
   - Windows: `start "" "<TouchDesigner.exe>" "<abs-path-to-tox>"`
...
5. **Poll until the bridge is live.** TouchDesigner startup takes tens of seconds (longer on first run). Poll TCP port 9981 every ~5 seconds for up to 3 minutes (e.g. `curl -s --max-time 3 -o /dev/null http://127.0.0.1:9981/` in a loop, run in the background rather than blocking). While waiting, tell the user that a license dialog or crash-recovery prompt on the TouchDesigner side blocks startup and must be dismissed manually.
...
If the download in step 2c fails, give the release URL and ask the user to download and unzip it manually, then re-run with the path as the argument. If the port never opens within the timeout: the most common causes are a modal dialog in TouchDesigner (license, update, crash recovery — ask the user to look at the TD window), the WebServer DAT bound to a non-default port (hand off to `/td-companion:td-setup`), or the tox failing to initialize (ask the user to check TouchDesigner's Textport for Python errors and report them back). Never kill or restart a running TouchDesigner process without explicit user consent — unsaved work may be open.
