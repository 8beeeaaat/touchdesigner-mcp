...
class APIControllerOpenAPI(IController):
	...
	def _handle_get_nodes(
		...
		**kwargs,
		...
		include_properties = (
			includeProperties if includeProperties is not None else False
		)

		service_result = self._service.get_nodes(
			parentPath, pattern, include_properties
		)
	...
	def _handle_update_node(self, body: str, **kwargs) -> Result:
		...
		service_result = self._service.update_node(node_path, properties)
