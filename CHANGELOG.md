...
**A malformed `--port` now stops the server instead of being ignored.** `--port` and `--mcp-http-port` accepted anything `Number.parseInt` could make a number of, so `--port=abc` silently connected to the default 9981 and `--port=9981junk` to 9981 as though it had been asked for. Both now print what was wrong and exit 1. If a config file carries a malformed port, the server will fail to start on this release rather than quietly using another one — which is the point, but it is a visible change. Unrecognized `--flags` are no longer discarded in silence either; they warn on stderr and are still ignored ([#211](https://github.com/8beeeaaat/touchdesigner-mcp/pull/211)).
...
**Codex users can install from the GitHub marketplace without building the plugin.** Run `codex plugin marketplace add 8beeeaaat/touchdesigner-mcp`, then `codex plugin add touchdesigner@touchdesigner-openai`. Node.js is still required; the client starts the bundled server automatically. ChatGPT Web needs a separately configured connection to the local TouchDesigner server; the [OpenAI plugin guide](docs/openai-plugin.md) covers Secure MCP Tunnel and registered connections. These plugin additions do not introduce any further MCP API changes ([#234](https://github.com/8beeeaaat/touchdesigner-mcp/pull/234)).
...
- **A Codex marketplace package and ChatGPT connection packaging**, reusing the Claude plugin's eight skills and bundled references/scripts. The ready-to-install OpenAI package is committed under `plugins/touchdesigner/` and registered by `.agents/plugins/marketplace.json`. It uses client-independent tool discovery and explicit mutation verification, and setup/launch handle clients without local shell access. English and Japanese READMEs now provide separate Claude Code and Codex installation sections ([#234](https://github.com/8beeeaaat/touchdesigner-mcp/pull/234)).
...
- `--stdio` is gone from every documented install path — the README, both installation guides, the architecture doc and the MCPB manifest. The flag was never parsed, so nothing behaves differently; but with unrecognized flags now warning, leaving it in place would have printed a spurious line on every start ([#211](https://github.com/8beeeaaat/touchdesigner-mcp/pull/211)).
...
- Every `--flag=value` argument was read with `split("=")[1]`, which keeps only the text up to a *second* `=` and drops the rest. `--host=http://h/?token=abc` silently became `http://h/?token`, and a port validated after that truncation — so `--port=9981=junk` passed as a clean `9981`. Arguments are now read whole, and a port has to be decimal digits end to end ([#211](https://github.com/8beeeaaat/touchdesigner-mcp/pull/211)).
- `describe_td_tools` never emitted a tool's `example`, in any detail level or response format — the only code reading the field handed it over as text that neither output path renders, so thirteen examples were written and maintained for nothing. Examples now appear at `detailLevel: "detailed"`, which costs a measured 3613 bytes (~900 tokens) on that level alone ([#226](https://github.com/8beeeaaat/touchdesigner-mcp/issues/226)).
- Five places in the OpenAPI schema hung keywords off a `$ref`, which OpenAPI 3.0 discards — three of them a `nullable: true`, dropped before any reader saw it. All five now wrap the reference in `allOf`, so the keyword survives parsing; the generated client is byte-identical, and the generated types and Zod schemas read these fields as nullable. A spec-strict 3.0 validator still will not, because `nullable` applies only to a `type` declared beside it and 3.0 has no way to say "this `$ref`, or null" — that needs `openapi: 3.1.0` and `type: [object, "null"]`, which is a toolchain migration rather than a schema edit ([#228](https://github.com/8beeeaaat/touchdesigner-mcp/issues/228)).

- Custom OpenAI plugin builds normalize a trailing slash in `--host` and reject embedded ports, paths, queries, and fragments before writing output, preventing malformed TouchDesigner connection URLs. Supply the port separately with `--port` ([#234](https://github.com/8beeeaaat/touchdesigner-mcp/pull/234)).
...
- Established by measurement, and recorded in the service docstring, that a Script OP callback exception reaches **no** operator-level stream — not `errors()`, `warnings()`, `scriptErrors()`, nor an `errorDAT` scoped over it. It goes to stderr alone. An earlier note calling `scriptErrors()` a likely home for it was wrong and is replaced. The remedy the tool contract recommends — catching in the callback and calling `scriptOp.addError(msg)` — was verified live and lands on `errors()` in the format the parser expects ([#219](https://github.com/8beeeaaat/touchdesigner-mcp/issues/219), still open for automatic capture).
...
- Maintainers refresh the committed OpenAI package with `npm run plugin:sync`; CI runs `npm run plugin:check` to reject stale, missing, or extra distribution files. Packaging tests cover host validation, shared resources, and switching between local MCP and registered ChatGPT connections without leaving stale configuration. Custom builds remain available through `npm run plugin:openai` ([#234](https://github.com/8beeeaaat/touchdesigner-mcp/pull/234)).

### Contributors

- [@8beeeaaat](https://github.com/8beeeaaat) — added Codex marketplace distribution and ChatGPT connection packaging ([#234](https://github.com/8beeeaaat/touchdesigner-mcp/pull/234)).
