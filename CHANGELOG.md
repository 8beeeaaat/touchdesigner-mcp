...
- The Claude Code plugin's bundled server failed to start with `CONNECTION_CLOSED` whenever Claude Code ran inside this repository's checkout. Claude Code spawns stdio servers in the working directory, and npx resolves `--package=touchdesigner-mcp-server@^2` against that directory's project tree before installing — this repository's own `package.json` satisfies the range, so npx skipped the install and then could not find the bin (`sh: touchdesigner-mcp-server: command not found`). The plugin now passes `--prefix=${CLAUDE_PLUGIN_ROOT}` so npx resolves the package from the plugin root, which has no `package.json`, wherever the session started. Sessions started in a project that merely depends on a 2.x release did not fail, but silently ran that project's copy instead of the latest 2.x; they now get the latest 2.x too. Any other directory was unaffected. The Codex package uses plugin-relative `cwd: "."` and `--prefix=.` to provide the same isolation, including when the plugin itself is nested inside a checkout.
